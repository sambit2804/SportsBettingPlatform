...
export const CONTRACT_ADDRESS =
  "CDNCR2CX3SNQRR6FCYLO46IDBT2FOQM6LWUC4WOAPJKWFABVDHGNO525";
...
// ============================================================
// Sports Betting Platform — Contract Methods
// ============================================================

/**
 * Place a bet on a match.
 * Calls: place_bet(user: Address, match_id: u32, team: Symbol, amount: i128)
 */
...
/**
 * Get bet details for a user on a specific match (read-only).
 * Calls: get_bet(user: Address, match_id: u32) -> Option<(Symbol, i128)>
 * Returns: [team, amount] or null
 */
export async function getBet(
  matchId: number,
  caller?: string
) {
  const callerAddress = caller || "GBGDZVPMKUYHQLK4CXJILM7JCX6MJDUV3UQUSYVYEA4C4ZT7JQR6I2V";
  return readContract(
    "get_bet",
    [toScValAddress(callerAddress), toScValU32(matchId)],
    caller
  );
}

/**
 * Declare the winner of a match (admin function).
 * Calls: declare_winner(match_id: u32, winning_team: Symbol)
 */
export async function declareWinner(
  ...
  return callContract(
    "declare_winner",
    [toScValU32(matchId), toScValSymbol(winningTeam)],
    caller,
    true
  );
}
 ...
 * Calls: claim_reward(user: Address, match_id: u32) -> bool
...
export async function claimReward(
  ...
  return callContract(
    "claim_reward",
    [toScValAddress(caller), toScValU32(matchId)],
    caller,
    true
  );
}
